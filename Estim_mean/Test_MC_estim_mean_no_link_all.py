...
####### Import liraries 
import Estim_mean_no_link_bench
import Estim_mean_no_link_1_n
import Estim_mean_no_link_pass
import Estim_mean_no_link_saga
import numpy as np 
...
Error = lambda x : Estim_mean_no_link_bench.error_1(h_0theo,x)
...
for n in range(NbSimu):#  n = 0
    ...
    h_01,error_h01 = Estim_mean_no_link_bench.Loop_super1(nb_episode,pdic,gamma= gamma,s_value=s_value,Error=Error,freq_print=freq_print,h_0=h_0,print_option =print_option)

    h_0 = np.array(h_0_init) # 100*np.random.rand(pdic['nb_iter'])#
    h_01_bis,error_h01_bis = Estim_mean_no_link_1_n.Loop_super1_bis(nb_episode,pdic,gamma= gamma,s_value=s_value,Error=Error,freq_print=freq_print,h_0=h_0,print_option =print_option)
    ...
    h_06,h_0_past6,nb_past6,error_h06 = Estim_mean_no_link_saga.Loop_super4(nb_episode,_exp_mean,pdic,inner_loop_func= Estim_mean_no_link_saga.Loop4_2,n_max=n_max,gamma= gamma,s_value=s_value,Error = Error,freq_print=freq_print,h_0=h_0,print_option =print_option)
    ...
    alpha_0 = 1
    alpha_max = 3
    h_012,h_0_past12,nb_past12,error_h012 = Estim_mean_no_link_pass.Loop_super5(nb_episode,_exp_mean,pdic,inner_loop_func= Estim_mean_no_link_pass.Loop10,n_max=n_max,gamma= gamma,s_value=s_value,Error = Error,freq_print=freq_print,h_0=h_0,alpha_0 = alpha_0, alpha_max = alpha_max,print_option =print_option)
...
option_save = ""
path_Image = Path_parent_directory + "\\Image"; ImageName = "\\improvement_drift_estim_v2_" + str(NbSimu) + "_simu_f"
...
Estim_mean_no_link_bench.Plot_plot(df,labels, xlabel ="Number of iterations", ylabel ="Log L2 - error",
