...
### Import libraries
import numpy as np
import Estim_mean_no_link_bench_2 as Estim_mean_bench
import Estim_mean_no_link_1_n_2 as Estim_mean_1_n
import Estim_mean_no_link_saga_2 as Estim_mean_saga
import Estim_mean_no_link_pass_2 as Estim_mean_pass
...
def plot_estim_quick(gamma, gamma_values = [0.1 , 0.09, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03, 0.02, 0.01], option_save = "", path_Image = "", ImageName = ""):
    i = np.abs(np.array(gamma_values)-gamma).argmin()
    gamma_close = round(gamma_values[i],3)
    ### load
    res = np.load(Current_directory +'\\Results\\res_gamma'+str(gamma_close)+'.npy').flatten().reshape((5,-1))
    ### plot
    index = res[0,:]
    error1 = res[1,:]
    error2 = res[2,:]
