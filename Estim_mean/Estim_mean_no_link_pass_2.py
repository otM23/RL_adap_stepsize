...
#### Inner loops
### Alg pass : increase gamma same sign
def Loop10(h_0,h_0_past,nb_past,exp_mean,pdic,n_max=2,gamma= 0.2,s_value=0,Error=None, alpha_0 = 1, alpha_max = 3, alpha_init = 1):
    s_value_next = 0
    ...
    for i in range(pdic['nb_iter']): # i = 0
        ...
        elif (h_0_cum[i]*h_0_past[i]>=0):
            alpha = min(alpha +1,alpha_max)
            h_0[i] = h_0[i] + alpha * gamma* h_0_cum[i]
        elif (h_0_cum[i]*h_0_past[i]<0):
            alpha = max(alpha -1,alpha_init)#alpha_0
            h_0[i] = h_0[i] + alpha* gamma * h_0_cum[i]
...
#### Super loops
### Alg 2 : pass ; this function takes the inner loop function as an argument
def Loop_super5(nb_episode,_exp_mean,pdic,inner_loop_func = Loop10,n_max=2,gamma= 0.2,s_value=0,freq_print=100,Error=None,h_0 = None, print_option = True, alpha_0 = 1, alpha_max = 3, pctg_0 = 0.1, alpha_init = 1):
    if (h_0 is None) :
    ...
    for ep in range(nb_episode):# ep = 0
        h_0,h_0_past,nb_past,error_val = inner_loop_func(h_0,h_0_past,nb_past,exp_mean,pdic,gamma= gamma_0,s_value=s_value,n_max=n_max,Error=Error, alpha_0 = alpha, alpha_max = alpha_max_, alpha_init = int(alpha_0))
        ...
        if (((ep %(freq_print))==freq_print-1) and (ep> 0)):
            ...
            count_period += 1
    if (count_within==0):
        return  [h_0,h_0_past,nb_past,mean_reward,gamma_0]
    else:
        val = np.array((count_reward*freq_print,error_within[:count_within].mean())).reshape((-1,2))
        return [h_0,h_0_past,nb_past,np.concatenate((mean_reward,val)),gamma_0]       