...
###### Benchmark inner loop : gamma may vary within the innner loop
def Lob_simu_inner_loop1_bis(h_0,h_0_past,h_0_Stay,h_0_Mkt,Lob_state_0,Intens_val,nb_iter, write_option = True, size_q = 2, gamma = 0.1, Error = None, reward_exec = None, eta = 1, nb_init = 1):
    ...
    for n in range(nb_iter): # n = 0
        ...
        ### Update h_0
        nb_init_bis = nb_init
        h_0[Lob_state_before[0],Lob_state_before[1]+1] += (gamma/(nb_init_bis))*h_0_cum[Lob_state_before[0],Lob_state_before[1]+1]
        h_0_Stay[Lob_state_before[0],Lob_state_before[1]+1] += (gamma/(nb_init_bis))*h_0_cum_stay[Lob_state_before[0],Lob_state_before[1]+1]
        h_0_Mkt[Lob_state_before[0],Lob_state_before[1]+1] += (gamma/(nb_init_bis))*h_0_cum_mkt[Lob_state_before[0],Lob_state_before[1]+1]
...
###### Benchmark super loop :  gamma may vary 
def Lob_simu_super_Loop1_bis(nb_episode,Intens_val,nb_iter, gamma= 0.2,Lob_state_0=None,freq_print=100,size_q = 2,Error=None, write_option = False, h_0 = None, reward_exec = None, eta = 1, h_0_Stay = None, h_0_Mkt = None, print_option =  True, pctg_0 = 0.1):
    ...
    for ep in range(nb_episode):
        if (ep %(10) ==9) :
            nb_init_0 = max((1 + (ep//10))*0.4,1)
        h_0,h_0_past,error_val,h_0_Stay,h_0_Mkt = Lob_simu_inner_loop1_bis(h_0,h_0_past,h_0_Stay,h_0_Mkt,Lob_state_0,Intens_val,nb_iter, write_option = write_option, size_q = size_q, gamma = gamma_0, Error = Error, reward_exec = reward_exec, eta = eta,nb_init=nb_init_0)['Result']
