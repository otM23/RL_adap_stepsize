...
import Projection_operators as proj_op
...
#    nb_past = np.zeros(((pdic['nb_iter']+1),pdic['size_q']),dtype = int)
    ...
    for ep in range(nb_episode): # ep = 0
        s_init = 0 # choose_elt_rnd(pdic['size_s'],pdic['s_min'],pdic['Step_s']) # 0#5 #choose_elt(pdic['size_s']+1,pdic['s_min'],pdic['Step_s'],{'index':5})
        x_init = 1 # choose_elt_rnd(pdic['size_x'],pdic['x_min'],pdic['Step_x']) # 0 #choose_elt(pdic['size_x']+1,pdic['x_min'],pdic['Step_x'],{'index':5})
