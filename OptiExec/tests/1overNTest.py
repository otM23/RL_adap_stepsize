# -*- coding: utf-8 -*-
"""
Created on Wed Oct 16 23:18:51 2019
...
import oneOverN as opti_exe_1_n
...
########### Benchmark : gamma vary within loop
...
option_save = ""
if option_save == "save":
    pathfile = Path_parent_directory + "\\Data"; fileName_01_bis = "\\improvement__optimal_execution_v_01_bis"
    fileName_01_bis_v = "\\improvement__optimal_execution_values_01_bis"
    np.save(pathfile + fileName_01_bis, np.array(error_v01_bis))
    np.save(pathfile + fileName_01_bis_v, np.array(v_01_bis))
option_load = "never"
if option_load == "load__carefully":
    error_v01_bis_01_bis = np.load(pathfile + fileName_01_bis + '.npy')
    v01_bis_01_bis = np.load(pathfile + fileName_01_bis_v + '.npy')


###############################################################################
########################### 2- Plot the improvement ############################
###############################################################################
...
df = [ [error_v01_bis[start:end,0],np.log(error_v01_bis[start:end,1])]]
labels = ["1/n"]
mark = ['']
...
##### Plot the values
option_save = ""
path_Image = Path_parent_directory + "\\Image"; ImageName = "\\improvement__optimal_execution_01_bis_210000_iter"; xtitle="v_01_bis"
delta = 1
#v_cj_rl = v_01.flatten() #v_reduced_bis.flatten()
v_cj_rl = v_01_bis[:,delta:-delta].flatten() #v_reduced_bis.flatten()
#q_values = np.arange(-pdic['Q_max'],pdic['Q_max'],Step_q)
q_values = np.arange(Q_min + delta * Step_q,pdic['Q_max'] - delta * Step_q,Step_q)
time_values = np.arange(0,T_max+Time_step,Time_step)
bins = [time_values.shape[0],q_values.shape[0]]
