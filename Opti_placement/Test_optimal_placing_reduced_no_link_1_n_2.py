...
####### Import liraries 
import Optimal_placing_reduced_no_link_1_n_2 as op_place_1_n_2
import numpy as np 
import pandas as pd
...
nb_iter_scheme = 400
reward_exec_1 = lambda qsame,bb_pos: op_place_1_n_2.reward_exec(qsame, bb_pos, gain = 6, cost_out = -0.6, cost_stay = -0.2)
df_bis = op_place_1_n_2.Sol_num_scheme(nb_iter_scheme,size_q,Intens_val_bis,tol = tol,reward_exec_1 = reward_exec_1)
...
df = [ [error_h01_bis[start:end,0],np.log(error_h01_bis[start:end,1])]]
labels = ["Benchmark "]
mark = ['o']
...
a = 1; b = 1; subplot0 = 1; Nset_tick_x = False; annot =True; fmt = '.2g'; cbar = False; annot_size = 16
cmaps = [sns.diverging_palette(10,240,n=6)]; masks = [None]
q_val_ = np.around(h_01_bis[1:,2:], decimals=1)
...
a = 1; b = 1; subplot0 = 1; Nset_tick_x = False; annot =True; fmt = '.2g'; cbar = False; annot_size = 16
cmaps = [sns.diverging_palette(10,240,n=6)]; masks = [None]
q_val_ = (h_01_Stay_bis >= h_01_Stay_bis)[1:,2:].astype(float)
for qsame in range(size_q-1): # qsame is the size
