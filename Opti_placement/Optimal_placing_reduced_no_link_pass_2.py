# -*- coding: utf-8 -*-
"""
Created on Tue Oct 15 23:26:25 2019
...
#### Inner loops  
###### Alg 2 inner loop : increase gamma same sign v2 : of the proximal gradient algorithm : 
def Lob_simu_inner_loop11(h_0,h_0_Stay,h_0_Mkt,h_0_past,Lob_state_0,Intens_val,nb_iter, write_option = True, size_q = 2, gamma = 0.1, Error = None, reward_exec = None, nb_past = None, eta = 1, alpha_0 = np.ones(2), alpha_base = 1, alpha_max = 10, r= 0.4):
    ...
    for n in range(nb_iter):# n = 0
        ...
        ############### End part observe next state ################################

        ##### The update m for the control stay
        ...
        ##### The update m for the control market
        ...
        ### update h_0, action stay
        j = nb_past[Lob_state_before[0],Lob_state_before[1]+1] # np.random.randint(self._n_max)  
        if j == 0:
            ### update h_0
            gamma_move = alpha[1] # (1 + r * (alpha - 1))
            h_0_Stay[Lob_state_before[0],Lob_state_before[1]+1] += gamma*gamma_move*(h_0_cum_stay[Lob_state_before[0],Lob_state_before[1]+1])
        elif (h_0_cum_stay[Lob_state_before[0],Lob_state_before[1]+1]*h_0_past[Lob_state_before[0],Lob_state_before[1]+1,1]>=0):
            alpha[1] = min(alpha[1] +1,alpha_max)
            gamma_move = (1 + r * (alpha[1] - 1)) # alpha # (1 + r * (alpha - 1))
            ### update h_0
            h_0_Stay[Lob_state_before[0],Lob_state_before[1]+1] += gamma*gamma_move*(h_0_cum_stay[Lob_state_before[0],Lob_state_before[1]+1])
        elif (h_0_cum_stay[Lob_state_before[0],Lob_state_before[1]+1]*h_0_past[Lob_state_before[0],Lob_state_before[1]+1,1] < 0):
            ### reinitialization
            h_0_past[Lob_state_before[0],Lob_state_before[1]+1,1] = 0
            alpha[1] = max(alpha[1] -1,alpha_base) # alpha_0[1] # alpha_base # max(alpha -1,1)#1 #
            gamma_move = alpha[1] # (1 + r * (alpha - 1))
            ### update h_0
            h_0_Stay[Lob_state_before[0],Lob_state_before[1]+1] += gamma*gamma_move*(h_0_cum_stay[Lob_state_before[0],Lob_state_before[1]+1])

        ## update h_0, action market
        if j == 0:
            ### update h_0
            gamma_move = alpha[0] # (1 + r * (alpha - 1))
            h_0_Mkt[Lob_state_before[0],Lob_state_before[1]+1] += gamma*gamma_move*(h_0_cum_mkt[Lob_state_before[0],Lob_state_before[1]+1])
        elif (h_0_cum_mkt[Lob_state_before[0],Lob_state_before[1]+1]*h_0_past[Lob_state_before[0],Lob_state_before[1]+1,0]>=0):
            alpha[0] = min(alpha[0] +1,alpha_max)
            gamma_move = (1 + r * (alpha[0] - 1)) # alpha # (1 + r * (alpha - 1))
            ### update h_0
            h_0_Mkt[Lob_state_before[0],Lob_state_before[1]+1] += gamma*gamma_move*(h_0_cum_mkt[Lob_state_before[0],Lob_state_before[1]+1])
        elif (h_0_cum_mkt[Lob_state_before[0],Lob_state_before[1]+1]*h_0_past[Lob_state_before[0],Lob_state_before[1]+1,0] < 0):
            ### reinitialization
            h_0_past[Lob_state_before[0],Lob_state_before[1]+1,0] = 0
            alpha[0] = max(alpha[0] -1,alpha_base) # alpha_0[1] # alpha_0[1] # alpha_base  # max(alpha -1,1)#1 #
            gamma_move = (1 + r * (alpha[0] - 1)) # alpha[0] # (1 + r * (alpha - 1))
            ### update h_0
            h_0_Mkt[Lob_state_before[0],Lob_state_before[1]+1] += gamma*gamma_move*(h_0_cum_mkt[Lob_state_before[0],Lob_state_before[1]+1])
        
        
        ## update the value function
        h_0[Lob_state_before[0],Lob_state_before[1]+1] = max(h_0_Stay[Lob_state_before[0],Lob_state_before[1]+1],h_0_Mkt[Lob_state_before[0],Lob_state_before[1]+1]) # gamma*gamma_move*(h_0_cum[Lob_state_before[0],Lob_state_before[1]+1])
        ...
        nb_past[Lob_state_before[0],Lob_state_before[1]+1] = 1
    ...
    error_val = Error(h_0)        
    if write_option:
        return {'lob':Lob_state, 'History':df_res ,'Result':[h_0,error_val,nb_past,h_0_Stay,h_0_Mkt,alpha]}
    else:
        return {'lob':Lob_state, 'History':pd.DataFrame() ,'Result':[h_0,error_val,nb_past,h_0_Stay,h_0_Mkt,alpha]}

#### Super loops
###### Alg 1 super loop : average value over the past observations : this function takes the inner loop function as an argument
def Lob_simu_super_Loop4(nb_episode,Intens_val,nb_iter,inner_loop_func = Lob_simu_inner_loop11, gamma= 0.2,Lob_state_0=None,freq_print=100,size_q = 2,Error=None, write_option = False, h_0 = None, reward_exec = None, nb_past = None, eta = 1, h_0_Stay = None, h_0_Mkt = None, print_option =  True, alpha_0 = 1, alpha_max = 10, r= 0.4, pctg_0 = 0.1):
#    Intens_val = Intens_val_bis
#    reward_exec = reward_exec_1    
...
#    gamma_0 = float(gamma)
    ...
    alpha = int(alpha_0)*np.ones(2)
    alpha_max_ = float(alpha_max)
    r_0 = float(r)
    ...
    for ep in range(nb_episode):
        h_0,error_val,nb_past,h_0_Stay,h_0_Mkt,alpha = inner_loop_func(h_0,h_0_Stay,h_0_Mkt,h_0_past,Lob_state_0,Intens_val,nb_iter, write_option = write_option, size_q = size_q, gamma = gamma_0, Error = Error, reward_exec = reward_exec, nb_past = nb_past, eta = eta, alpha_0 = alpha, alpha_base = int(alpha_0), alpha_max = alpha_max_,r= r_0)['Result']
